...
def homepage(request):
	...
	listas = Coleccion.objects.all().exclude(nombre__contains='Magazine').order_by('-id')
	return render(request,'homepage.html',{'articles':articles,'listas':listas,'npages':range(npages)})
...
def category(request,cat,p):
	...
	if int(cat) in [1,2,3,4,5,14,15,17,18,19]:
		nlist = 25
...
def item(request,iid):
    ...
    more_creditos = sorted(RelItemPersona.objects.exclude(item__id=thisItem.id).filter(persona__id__in = personas_id).distinct(), key=lambda t: t.item.pubyear)
...
def startitem(request,iid):
	...
	if thisItem.item_cat.id in [1,17,18]:
		units = 'pages'
...
def coleccion(request,id):
	...
	if request.method == 'POST':
		...
		return render(request,'list.html',{'col':col,'ci':colitms,'listas':listas,'rb':rb,'nb':nb,'por_r':por_r})
...
def addPartido(request,lid):
	...
	if request.method == 'POST':
		...
		return render(request,'add-partido.html',{'liga':liga,'equipos':equipos,'last_m':last_mr})
	else:
		last_mr = Partido.objects.latest('id')
		return render(request,'add-partido.html',{'liga':liga,'equipos':equipos,'last_m':last_mr})
...
def viewLiga(request,sta,lig):
	...
	if sta == "1":
		...
		matches = Partido.objects.filter(terminado=False,liga__id=int(lig)).order_by('fecha')
...
def viewMatch(request,pid):
	...
	if request.method == 'POST':
		...
		return render(request,'partido.html',{'matches':matches,'partido':partido,'jlocal':jugadores_local,'jvisit':jugadores_visit,'lgoles':lgoles,'vgoles':vgoles,'lpens':lpens,'vpens':vpens,'ligas':ligas})
...
def addContract(request,pid):
	...
	newJugador = Jugador.objects.create(nombre=nombre,pais=pais)
	newJugador.save()

	newC = Contrato.objects.create(jug=newJugador,equ=equipo,active=True)
	newC.save()
...
def quemar(request,itm):
	...
	newC = Consumo.objects.create(con_itm=thisItem,unidades="quemado",cantidad=0,fecha_inicio="2000-01-01",fecha_fin="2000-01-01")
	newC.save()
...
def sccteam(request,t):
	...
	ligas = Liga.objects.all().order_by('-id')
	matches = Partido.objects.filter(Q(visita__id=int(t)) | Q(local__id=int(t))).exclude(terminado=False).order_by('-fecha')
	contratos = Contrato.objects.filter(equ__id=int(t))


	return render(request,'sccteam.html',{'matches':matches,'ligas':ligas,'thisEquipo':thisEquipo,'contratos':contratos})
...
def jugador(request,jid):
	...
	if request.method == 'POST' and request.POST.get("team","0") != "0":
		...
		newC = Contrato.objects.create(jug=jug,equ=equipon)
		newC.save()
		...
		newC = Contrato.objects.create(jug=newJ,equ=equipon)
		newC.save()
...
def addpartidocomm(request):
	...
	newC = PartidoComment.objects.create(comm_partido=obj_partido,comm=comm)
	newC.save()
...
def addBaseballTeam(request):
    if request.method == 'POST':
        eq = request.POST.get("nombre")
        ct = request.POST.get("ciudad")
        lg = request.FILES.get("logo")
        newBT = baseballTeam.objects.create(nombre=eq,ciudad=ct,logo=lg)
        newBT.save()
        return render(request,'add-baseball-team.html',{})
    else:
        return render(request,'add-baseball-team.html',{})

def addBaseballGame(request):
    cats = baseballTeam.objects.all().order_by('nombre')
    if request.method=='POST':
        dt = request.POST.get("gameDate")
        lr = request.POST.get("l_runs")
        vr = request.POST.get("v_runs")
        fs = request.POST.get("fase")
        inns = request.POST.get("innings")
        st =request.POST.get("stadium")
        loc = baseballTeam.objects.get(pk=int(request.POST.get("local")))
        vis = baseballTeam.objects.get(pk=int(request.POST.get("visit")))
        newG = baseballGame.objects.create(local=loc,visit=vis,l_runs = 0,v_runs=0,innings=inns,gameDate=dt,fase=fs,stadium=st)
        newG.save()
        return redirect('/mlbgames/')
    else:
        return render(request,'add-baseball-game.html',{'cats':cats})

def mlbGames(request):
    matches = baseballGame.objects.filter(Q(l_runs__gt=0) | Q(v_runs__gt=0)).all().order_by('-gameDate')[0:25]
    seasons = baseballGame.objects.values('fase').annotate(ngames=Count('fase'))

    last_match = baseballGame.objects.latest('id')

    fase_now = last_match.fase

    season_matches = baseballGame.objects.filter(Q(fase=fase_now) & (Q(l_runs__gt=0) | Q(v_runs__gt=0))).order_by('-gameDate')
    ...
    for sm in season_matches:
    	conteo = conteo + 1
    	if sm.local.id == 1 and sm.l_runs > sm.v_runs:
    		wins = wins + 1
    	elif sm.visit.id == 1 and sm.v_runs > sm.l_runs:
    		wins = wins + 1

    	if (conteo <= 7) and ((sm.local.id == 1 and sm.l_runs > sm.v_runs) or (sm.visit.id == 1 and sm.v_runs > sm.l_runs)):
    		ult_juegos = ult_juegos+"&#9989;"
    	elif (conteo <= 7) and ((sm.local.id == 1 and sm.l_runs < sm.v_runs) or (sm.visit.id == 1 and sm.v_runs < sm.l_runs)):
    		ult_juegos = ult_juegos+"&#10060;"

    loses = conteo-wins

    avg = round((wins*1.000)/(conteo*1.00),3)

    marcador_global = {'games':conteo,'wins':wins,'loses':loses,'avg':avg,'uj':ult_juegos }

    return render(request,'mlb-games.html',{'matches':matches,'seasons':seasons,'mg':marcador_global})

def mlbGame(request,id):
    match = baseballGame.objects.get(pk=int(id))
    seasons = baseballGame.objects.values('fase').annotate(ngames=Count('fase'))

    atbat = baseballGameBat.objects.filter(bGame=match)
    pitch = baseballGamePitch.objects.filter(bGame=match)
    total_runs = match.l_runs+match.v_runs

    return render(request,'mlb-game.html',{'m':match,'seasons':seasons,'atbat':atbat,'pitch':pitch,'total_runs':total_runs})

def atbat(request,gameid):
    game = baseballGame.objects.get(pk=int(gameid))
    c_local = baseballContract.objects.filter(bTeam__id=game.local.id).order_by('bPlayer__nombre')
    c_visit = baseballContract.objects.filter(bTeam__id=game.visit.id).order_by('bPlayer__nombre')

    if request.method=='POST':

        con = baseballContract.objects.get(pk=int(request.POST.get("contract")))
        ab=int(request.POST.get("ab"))
        ...
        newAB = baseballGameBat.objects.create(bGame=game,contract=con,ab=ab,r=r,h=h,bb=bb,cp=cp,hr=hr)
        newAB.save()

        return redirect("/mlbgame/{}#batTable".format(int(game.id)))
    else:
        return render(request,'at_bat.html',{'game':game,'c_local':c_local,'c_visit':c_visit})

def pitching(request,gameid):
    game = baseballGame.objects.get(pk=int(gameid))
    c_local = baseballContract.objects.filter(bTeam__id=game.local.id).order_by('bPlayer__nombre')
    c_visit = baseballContract.objects.filter(bTeam__id=game.visit.id).order_by('bPlayer__nombre')
    ...
    if request.method=='POST':
        ...
        newAB = baseballGamePitch.objects.create(bGame=game,contract=con,ip=ip,h=h,cl=cl,bb=bb,p=p)
        newAB.save()

        return redirect("/mlbgame/{}#pitchTable".format(int(game.id)))
    else:
        return render(request,'pitching.html',{'game':game,'c_local':c_local,'c_visit':c_visit})
...
def viewBaseballSeason(request,season):

    bat_stats = baseballGameBat.objects.filter(bGame__fase=season).values('contract__bPlayer__nombre').annotate(hits=Sum('h', output_field=FloatField()),atbat=Sum('ab', output_field=FloatField()),hrs=Sum('hr'),rbi=Sum('cp'),runs=Sum('r')).annotate(savg=Round((1000*Cast('hits',output_field=FloatField())/F('atbat')))).order_by('-atbat')

    seasons = baseballGame.objects.values('fase').annotate(ngames=Count('fase'))
    return render(request,'bb_season.html',{'season':season,'seasons':seasons,'bat_stats':bat_stats})
...
def mlbPendingGames(request):
    matches = baseballGame.objects.filter(Q(l_runs=0) & Q(v_runs=0)).all().order_by('gameDate')[0:25]
    seasons = baseballGame.objects.values('fase').annotate(ngames=Count('fase'))
    ...
    return render(request,'mlb-pending-games.html',{'matches':matches,'seasons':seasons,'mg':marcador_global})

def updateBaseballGame(request):
    thisGame = baseballGame.objects.get(pk=int(request.POST.get("gameid")))
    localRuns = int(request.POST.get("l_runs"))
    visitRuns = int(request.POST.get("v_runs"))
    baseballGame.objects.filter(pk=thisGame.id).update(l_runs=localRuns,v_runs=visitRuns)
    return redirect('/mlbgames/')
...
def addPersona(request):
	...
	if request.method=="POST":
		...
		return redirect('/view-ppl/{}/0'.format(request.POST.get("tipo")))
...
def viewPersona(request,p):
	...
	return render(request,'view-persona.html',{'ptitle':ptitle,'ppl':thisPerson,'nowItems':nowItems})
...
def editPersona(request,p):
	...
	if request.method == 'POST':
		...
		return render(request,'edit-persona.html',{'ptitle':ptitle,'ppl':thisPerson})
...
def finishNoteConsumo(request):
	...
	NoteMedia.objects.filter(pk=noteM.id).update(finish_date=request.POST.get("finish_date"))
