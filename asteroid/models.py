...
class Item(models.Model):
	...
	@property
	def author(self):
		q = ItemTable.objects.filter(tbl_itm__id=self.id,campo='author').count()

		if q == 0:
			pubyear = 0
		else:
			anhos = ItemTable.objects.filter(tbl_itm__id=self.id,campo='author')
			for an in anhos:
				pubyear = an.datos
...
class baseballContract(models.Model):
    bTeam = models.ForeignKey(baseballTeam,on_delete=models.CASCADE)
    bPlayer = models.ForeignKey(baseballPlayer,on_delete=models.CASCADE)

    def __str__(self):
        return self.bTeam.nombre+"-"+self.bPlayer.nombre

class baseballGame(models.Model):
    local = models.ForeignKey(baseballTeam, on_delete = models.CASCADE,related_name="local_t")
    visit = models.ForeignKey(baseballTeam, on_delete = models.CASCADE,related_name="visit_t")
    l_runs = models.IntegerField()
    v_runs = models.IntegerField()
    innings = models.IntegerField()
    gameDate = models.DateField()
    fase = models.CharField(max_length=128,null=True, blank=True)
    stadium = models.CharField(max_length=256,null=True, blank=True)

    def __str__(self):
        return self.local.nombre+" @ "+self.visit.nombre

class baseballGameBat(models.Model):
    bGame = models.ForeignKey(baseballGame, on_delete = models.CASCADE)
    contract = models.ForeignKey(baseballContract, on_delete = models.CASCADE)
    ...
    def __str__(self):
        return str(self.bGame.gameDate)+' '+self.contract.bPlayer.nombre

class baseballGamePitch(models.Model):
    bGame = models.ForeignKey(baseballGame, on_delete = models.CASCADE)
    contract = models.ForeignKey(baseballContract, on_delete = models.CASCADE)
